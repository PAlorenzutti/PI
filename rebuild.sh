...
cd "$BASE_DIR/$COMPOSE_DIR" && docker compose -p gerenciapet down
...
cd "$BASE_DIR/$COMPOSE_DIR" && docker compose -p gerenciapet up -d
sleep 10
...
gnome-terminal --tab --title="gerenciapet-frontend" -- bash -c '
  ...
  docker exec -it '"$CONTAINER"' bash -c "cd gerenciapet-frontend/ && npm install && ng build && ng serve --host 0.0.0.0"
  exec bash
' &
